...
let DUser = Donation.belongsTo(User, {foreignKey: 'donation_uid', targetKey: 'id', as: 'user'});
...
module.exports = {
  'POST /book/add': async (ctx, next) => {
    ...
    try {
        ...
        let newBook;
        if (book) {
            newBook = await Book.update({...book, ...bookData, stock: book.stock + 1, num: book.num + 1}, {
                where: {ISBN: ISBN}
            });
        } else {
        ...
        if (isDonate) {
            let condition = {where: {donation_uid: ctx.uid, book_isbn: ISBN}}
            let donation = await Donation.findOne(condition);
            if (donation) {
                await Donation.update({...donation, donation_num: donation.donation_num + 1}, condition);
            } else {
  ...
  'POST /book/op': async (ctx, next) => {
        ...
        try {
            ...
            let borrowCon = {where: {user_id: ctx.uid, book_isbn: ISBN, borrow_status: 1}};
            ...
            if (record) {
                ...
                if (type == 2) {
                    await BorrowRecord.update({...record, give_back_time: new Date(), borrow_status: 0}, borrowCon)
                }
            } else if(!record){
                if (type == 2) {
                    ctx.body = {
            ...
            await Book.update({...book, stock: stock}, {
                where: {ISBN: ISBN}
            });
            ctx.body = {
                code: 200,
    ...
    'GET /book/borrowRecord': async (ctx, next) => {
        let {page = 1, pageSize = 10, user_id, borrow_status = 1} = ctx.request.query;
        page = +page;
        pageSize = +pageSize;
        let condition = {
    ...
    'GET /book/detail/:isbn': async (ctx, next) => {
        ...
        try {
            let book = await Book.findOne({where: {ISBN: isbn}})
            ctx.body = {
                code: 200,
                data: book,
                msg: ''
